...
public class CustomerController {
    ...
    public String customerDashboard(Authentication authentication, Model model) {
        try {
            ...
            List<Booking> customerBookings = bookingService.getBookingsByCustomerName(currentUser.getFirstName() + " " + currentUser.getLastName());
    ...
    public String saveBooking(@RequestParam Map<String, String> allParams,
        ...
        try {
            ...
            // Create Booking object manually from form parameters
            ...
            String serviceType = allParams.get("serviceType");
            ...
            if (booking.getPaidAmount() == null) {
                ...
                if (payingAmountStr != null && !payingAmountStr.isEmpty()) {
                    try {
                        ...
                        // Calculate remaining amount using the captured service price
                        BigDecimal remainingAmount = servicePrice.subtract(payingAmount);
                        booking.setRemainingAmount(remainingAmount.max(BigDecimal.ZERO));
                    } catch (NumberFormatException e) {
                        // If invalid number, set to zero
                        booking.setPaidAmount(BigDecimal.ZERO);
                        booking.setRemainingAmount(servicePrice);
                    }
                } else {
                    booking.setPaidAmount(BigDecimal.ZERO);
                    booking.setRemainingAmount(servicePrice);
    ...
    public String submitFeedback(@PathVariable Long id,
        ...
        try {
            ...
            redirectAttributes.addFlashAttribute("message", "Thank you for your feedback! Your review has been submitted successfully.");
    ...
    public String deleteFeedback(@PathVariable Long id, Authentication authentication, RedirectAttributes redirectAttributes) {
        try {
            String username = authentication.getName();
            User currentUser = userService.getUserByUsername(username).orElse(null);
            if (currentUser == null) {
